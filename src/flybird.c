...
int flybird(int argc,char* argv[])
{
	...
	lcd_fill(0x000000ff);
	ShowBmp(0,0,"./bmp/flybird/background.bmp");
	int background = open("./bmp/flybird/background.bmp",O_RDONLY);
	if(background < 0)
	{
	...
	close(background);
	//ShowBGArray(0,0);
	savebirdbmp("./bmp/flybird/bird_up.bmp",bird_up_color);
	savebirdbmp("./bmp/flybird/bird_down.bmp",bird_down_color);
	...
	while(1)
	{
		...
		if(game_over)
		{
			ShowBmp(0,0,"./bmp/flybird/gameover.bmp");
			while(game_over);
			bird_y = 100;
			bird_score = 0;
			ShowBmp(0,0,"./bmp/flybird/background.bmp");
